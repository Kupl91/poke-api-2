...
const PokemonList: React.FC<PokemonListProps> = ({
  ...
  const emptyRows = new Array(
    5 - pokemons.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).length,
  ).fill(null)
        ...
        <TableBody>
                      ...
                      <DropdownMenuItem>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button
                              variant="outline"
                              onMouseOver={() => handleDetailsClick(pokemon.id)}
                          ...
                          <DropdownMenuContent sideOffset={4} className="p-1 bg-white shadow-md">
                            {selectedDetail && selectedDetail.id === pokemon.id && (
                              <>
                                ...
                                {selectedDetail.abilities &&
                                  selectedDetail.abilities.map(ability => (
                                    <DropdownMenuItem
                                      key={ability.ability.name}
                                    >{`Способность: ${ability.ability.name}`}</DropdownMenuItem>
                                  ))}
                              </>
                            )}
                          </DropdownMenuContent>
                            ...
                            {updatingPokemon && updatingPokemon.id === pokemon.id && (
                              <>
                                ...
                                <Input
                                  type="number"
                                  name="experience"
                                  value={updatingPokemon.experience || ''}
                                  onChange={handleUpdateInputChange}
                                  placeholder="Опыт"
                                  className="bg-gray-200"
                                />
                                <Button variant="outline" onClick={handleUpdateSubmit} className="bg-blue-500">
                                  Отправить
                                </Button>
                              </>
                            )}
          ...
          {emptyRows.map((_, index) => (
            <TableRow key={`empty-${index}`}>
              <TableCell colSpan={6} style={{ height: '73px' }}>
                &nbsp;
              </TableCell>
            </TableRow>
          ))}
