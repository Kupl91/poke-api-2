...
const FilterAndSort: React.FC<FilterAndSortProps> = ({
  ...
  const sortAttributes = ['name', 'weight', 'height', 'species', 'experience'].map(key => ({
    key: key as keyof Pokemon,
  ...
  return (
    <div className="space-y-4">
        ...
        <Select onValueChange={value => handleFilterTypeChange(value as keyof Pokemon)}>
          ...
          <SelectContent>
            {sortAttributes.map(attribute => (
              <SelectItem value={attribute.key} key={`filter-${attribute.key}`}>
                {attribute.label}
              </SelectItem>
            ))}
          </SelectContent>
      ...
      <Table className="min-w-full divide-y divide-gray-200">
          ...
          <TableRow>
            {sortAttributes.map((attribute, idx) => (
              <TableHead className="text-center" key={`sort-${attribute.key}`}>
                <button
                  onClick={() => handleSortChange(attribute.key)}
        ...
        <TableBody>{/* Здесь будут строки таблицы */}</TableBody>
      </Table>
    </div>
  )
}
